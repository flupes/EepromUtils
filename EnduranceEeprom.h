...
/**
   ...
   much more data space. This is the tradeoff to allow more data writes
   than the number of cycles the EEPROM can support. Typcally the ATmega
   chip will allow 100K writes. This can seems a lot, but imagine you need
   to store a value every minute to the EEPROM, after 70 days of runtime
   the EEPROM would be dead.


   To keep track of the current address of the data inthe circular buffer,
...
public:
  ...
  /** Initialize a Endurance Eeprom data structure.
      ...
      @note If endurFactor is 1, we have a degeranative scenario and the
      ...
      EnduranceBuffer to have a unified interface to the EEPROM with or
...
protected:
  ...
  /** Address of the begining of the status circular buffer. */
  uint16_t m_statusAddr;

  /** Address of the begining of the data circular buffer. */
  uint16_t m_dataAddr;
